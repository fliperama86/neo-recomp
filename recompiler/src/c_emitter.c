...
#define NG_EMIT_DEFAULT_FUNCTIONS_PER_SHARD 2048u
#define NG_EMIT_MAX_PATH 1024u
...
static void emit_prelude(FILE *out) {
    ...
    fprintf(out, "#include \"ngrecomp/generated_abi.h\"\n\n");
    ...
    fprintf(out, "#endif\n\n");
    fprintf(out, "#ifndef NG_GENERATED_CAT2\n");
    fprintf(out, "#define NG_GENERATED_CAT2(a, b) a##b\n");
    fprintf(out, "#endif\n\n");
    fprintf(out, "#ifndef NG_GENERATED_CAT\n");
    fprintf(out, "#define NG_GENERATED_CAT(a, b) NG_GENERATED_CAT2(a, b)\n");
    fprintf(out, "#endif\n\n");
    fprintf(out, "#ifndef NG_GENERATED_SHARD_STEP\n");
    fprintf(out, "#define NG_GENERATED_SHARD_STEP(name) NG_GENERATED_CAT(NG_GENERATED_CALL, name)\n");
    fprintf(out, "#endif\n\n");
    fprintf(out, "void NG_GENERATED_DISPATCH(uint32_t addr);\n\n");
}

static void emit_dispatch_state(FILE *out) {
...
static void emit_helper_functions(FILE *out) {
    ...
    fprintf(out, "    ng68k_write16(g_ng_m68k.a[7], saved_sr);\n");
    fprintf(out, "}\n\n");
    fprintf(out, "static int ng_service_reset(void) {\n");
    fprintf(out, "    uint32_t reset_pc = 0;\n");
    fprintf(out, "    uint32_t reset_ssp = 0;\n");
    fprintf(out, "    if (!ng_m68k_take_reset(&reset_pc, &reset_ssp)) return 0;\n");
    fprintf(out, "    g_ng_m68k.sr = 0x2700u;\n");
    fprintf(out, "    g_ng_m68k.ssp = reset_ssp;\n");
    fprintf(out, "    g_ng_m68k.a[7] = reset_ssp;\n");
    fprintf(out, "    g_ng_m68k.pc = reset_pc & 0x00FFFFFFu;\n");
    fprintf(out, "    NG_GENERATED_DISPATCH(g_ng_m68k.pc);\n");
    ...
    fprintf(out, "    int take_interrupt = 0;\n");
    fprintf(out, "    if (ng_service_reset()) return 1;\n");
    fprintf(out, "    take_interrupt = ng_m68k_take_interrupt(current_mask, &level, &vector);\n");
    fprintf(out, "    if (ng_service_reset()) return 1;\n");
    fprintf(out, "    if (!take_interrupt) return 0;\n");
...
typedef struct NgShardDispatchRoute {
    uint32_t addr;
    uint32_t shard;
} NgShardDispatchRoute;

static int compare_shard_dispatch_route(const void *a, const void *b) {
    const NgShardDispatchRoute *av = (const NgShardDispatchRoute *)a;
    const NgShardDispatchRoute *bv = (const NgShardDispatchRoute *)b;
    if (av->addr != bv->addr) {
        return (av->addr > bv->addr) - (av->addr < bv->addr);
    }
    return (av->shard > bv->shard) - (av->shard < bv->shard);
}

static void emit_dispatch_loop(FILE *out) {
...
static void emit_dispatch(FILE *out, const NgFunctionDiscovery *discovery) {
    ...
    fprintf(out, "    }\n");
    fprintf(out, "}\n\n");
    emit_dispatch_loop(out);
}

static int emit_sharded_dispatch(FILE *out,
                                 const NgFunctionDiscovery *discovery,
                                 uint32_t functions_per_shard,
                                 uint32_t shard_count) {
    NgShardDispatchRoute *routes =
        (NgShardDispatchRoute *)malloc((size_t)discovery->count *
                                       sizeof(*routes));
    uint32_t route_count = 0u;
    if (discovery->count != 0u && !routes) {
        return 0;
    }
    ...
    for (uint32_t i = 0; i < discovery->count; ++i) {
        ...
        routes[route_count].addr = discovery->addrs[i] & 0x00FFFFFFu;
        routes[route_count].shard =
            functions_per_shard == 0u ? 0u : i / functions_per_shard;
        ++route_count;
    }
    if (route_count != 0u) {
        qsort(routes,
              route_count,
              sizeof(*routes),
              compare_shard_dispatch_route);
    }

    fprintf(out, "\n");
    for (uint32_t i = 0; i < shard_count; ++i) {
        fprintf(out,
                "int NG_GENERATED_SHARD_STEP(_shard_%04u)(uint32_t addr);\n",
                (unsigned)i);
    }
    if (shard_count != 0u) {
        fprintf(out, "\n");
    }
    fprintf(out, "typedef struct NgGeneratedDispatchRoute {\n");
    fprintf(out, "    uint32_t addr;\n");
    fprintf(out, "    uint32_t shard;\n");
    fprintf(out, "} NgGeneratedDispatchRoute;\n\n");
    fprintf(out, "static const NgGeneratedDispatchRoute ng_generated_dispatch_routes[] = {\n");
    for (uint32_t i = 0; i < route_count; ++i) {
        fprintf(out,
                "    { 0x%08Xu, %uu },\n",
                routes[i].addr,
                (unsigned)routes[i].shard);
    }
    fprintf(out, "};\n\n");
    fprintf(out,
            "static const uint32_t ng_generated_dispatch_route_count = %uu;\n\n",
            (unsigned)route_count);
    fprintf(out, "static void NG_GENERATED_STEP(uint32_t addr) {\n");
    fprintf(out, "    addr &= 0x00FFFFFFu;\n");
    fprintf(out, "    g_ng_m68k.pc = addr;\n");
    fprintf(out, "    uint32_t lo = 0;\n");
    fprintf(out, "    uint32_t hi = ng_generated_dispatch_route_count;\n");
    fprintf(out, "    while (lo < hi) {\n");
    fprintf(out, "        uint32_t mid = lo + ((hi - lo) / 2u);\n");
    fprintf(out, "        if (ng_generated_dispatch_routes[mid].addr < addr) {\n");
    fprintf(out, "            lo = mid + 1u;\n");
    fprintf(out, "        } else {\n");
    fprintf(out, "            hi = mid;\n");
    fprintf(out, "        }\n");
    fprintf(out, "    }\n");
    fprintf(out, "    if (lo < ng_generated_dispatch_route_count && ng_generated_dispatch_routes[lo].addr == addr) {\n");
    fprintf(out, "        switch (ng_generated_dispatch_routes[lo].shard) {\n");
    for (uint32_t i = 0; i < shard_count; ++i) {
        fprintf(out,
                "        case %uu: (void)NG_GENERATED_SHARD_STEP(_shard_%04u)(addr); return;\n",
                (unsigned)i,
                (unsigned)i);
    }
    ...
    fprintf(out, "        }\n");
    fprintf(out, "    }\n");
    fprintf(out, "    ng_call_by_address(addr);\n");
    fprintf(out, "}\n\n");
    emit_dispatch_loop(out);
    free(routes);
    return ferror(out) == 0;
...
static int emit_instr(FILE *out,
    ...
    case NG_M68K_RTS:
        ...
        return 0;
    case NG_M68K_LINK:
        fprintf(out, "    { uint32_t ng_old_an = g_ng_m68k.a[%u];\n",
                instr->reg);
        fprintf(out, "      g_ng_m68k.a[7] -= 4u;\n");
        fprintf(out, "      ng68k_write32(g_ng_m68k.a[7], ng_old_an);\n");
...
static void emit_function_body(FILE *out,
    ...
    fprintf(out, "    uint16_t ng_trace_sr = 0;\n");
    fprintf(out, "    (void)ng_trace_sr;\n");
...
static int emit_function_range(FILE *out,
                               const NgProgramRom *rom,
                               const NgFunctionDiscovery *discovery,
                               uint32_t start_index,
                               uint32_t end_index,
                               NgEmitDiagnostics *diagnostics) {
    if (end_index > discovery->count) {
        end_index = discovery->count;
    }
...
static int emit_functions(FILE *out,
                          ...
                          const NgFunctionDiscovery *discovery,
                          NgEmitDiagnostics *diagnostics) {
    return emit_function_range(out,
                               rom,
                               discovery,
                               0u,
                               discovery->count,
...
static void emit_shard_declarations(FILE *out,
                                    const NgFunctionDiscovery *discovery,
                                    uint32_t start_index,
                                    uint32_t end_index) {
    if (end_index > discovery->count) {
...
static void emit_shard_step(FILE *out,
                            const NgFunctionDiscovery *discovery,
    ...
    for (uint32_t i = start_index; i < end_index; ++i) {
        ...
        ng_c_symbol_for_discovery_index(discovery,
                                        ...
                                        symbol,
                                        (unsigned)sizeof(symbol));
        fprintf(out, "    case 0x%08Xu: %s(); return 1;\n", addr, symbol);
    }
...
static int make_emit_path(char *out,
                          size_t out_size,
                          const char *dir,
                          const char *name) {
    int written;
    if (!out || out_size == 0u || !dir || !name) {
        return 0;
    }
...
static int emit_sharded_dispatch_file(const char *path,
                                      const NgFunctionDiscovery *discovery,
                                      uint32_t functions_per_shard,
                                      uint32_t shard_count) {
...
static int emit_shard_file(const char *path,
    ...
    emit_prelude(out);
    emit_helper_functions(out);
    emit_shard_declarations(out, discovery, start_index, end_index);
    emit_shard_step(out, discovery, shard_index, start_index, end_index);
    if (!emit_function_range(out,
                             rom,
                             discovery,
                             start_index,
                             end_index,
...
int ng_emit_c_shards(const char *out_dir,
    ...
    NgEmitDiagnostics *active_diagnostics =
        diagnostics ? diagnostics : &local_diagnostics;
    ...
    if (!out_dir || !rom || !discovery) {
        return 0;
    }
    if (functions_per_shard == 0u) {
        functions_per_shard = NG_EMIT_DEFAULT_FUNCTIONS_PER_SHARD;
    }

    ng_emit_diagnostics_init(active_diagnostics);
    if (!ensure_emit_dir(out_dir) ||
        !make_emit_path(dispatch_path,
                        sizeof(dispatch_path),
                        out_dir,
    ...
    shard_count = discovery->count == 0u
                      ? 0u
                      : (discovery->count + functions_per_shard - 1u) /
                            functions_per_shard;
    if (!emit_sharded_dispatch_file(dispatch_path,
                                    discovery,
                                    functions_per_shard,
                                    shard_count)) {
        return 0;
    ...
    for (uint32_t shard = 0; ok && shard < shard_count; ++shard) {
        char name[64];
        char shard_path[NG_EMIT_MAX_PATH];
        uint32_t start_index = shard * functions_per_shard;
        uint32_t end_index = start_index + functions_per_shard;
        if (end_index < start_index || end_index > discovery->count) {
            end_index = discovery->count;
        }
        snprintf(name, sizeof(name), "generated_shard_%04u.c", (unsigned)shard);
        if (!make_emit_path(shard_path,
                            sizeof(shard_path),
                            out_dir,
                            name) ||
            !emit_shard_file(shard_path,
                             rom,
                             discovery,
                             shard,
                             start_index,
