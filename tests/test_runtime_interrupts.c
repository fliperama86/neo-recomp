...
int main(void) {
    ...
    CHECK(ng_neogeo_watchdog_last_kick_poll() == ng_neogeo_interrupt_polls());
    ng68k_write8(0x00310001u, 0xA5u);
    CHECK(ng_neogeo_watchdog_kicks() == 2u);
    CHECK(ng68k_read8(NG_NEO_REG_DIPSW) == 0xFFu);
    ng_neogeo_reset_runtime();
    CHECK(ng_neogeo_watchdog_kicks() == 0u);
    CHECK(ng_neogeo_watchdog_timeout_polls() == 0u);
    CHECK(ng_neogeo_watchdog_resets() == 0u);
    CHECK(ng68k_read8(NG_NEO_REG_DIPSW) == 0xFFu);

    {
        ...
        ng_neogeo_set_watchdog_reset_vector(0x00123456u, 0x0010F300u);
        ng_neogeo_set_watchdog_timeout_polls(3u);
        CHECK(ng_neogeo_watchdog_timeout_polls() == 3u);
        CHECK(ng_neogeo_watchdog_last_kick_poll() == ng_neogeo_interrupt_polls());
        CHECK(!ng_m68k_take_interrupt(7, &level, &vector));
        CHECK(!ng_m68k_take_interrupt(7, &level, &vector));
        CHECK(!ng_neogeo_watchdog_reset_pending());
        ng68k_write8(NG_NEO_REG_SWPBIOS, 0xFFu);
        ng68k_write8(NG_NEO_REG_SRAMUNLOCK, 0xFFu);
        g_ng_m68k.pc = 0x00ABCDEFu;
        CHECK(!ng_m68k_take_interrupt(7, &level, &vector));
        CHECK(ng_neogeo_watchdog_reset_pending());
        CHECK(ng_neogeo_watchdog_last_reset_pc() == 0x00ABCDEFu);
        CHECK(ng_neogeo_watchdog_last_reset_poll() == ng_neogeo_interrupt_polls());
        CHECK(ng_m68k_take_reset(&reset_pc, &reset_ssp));
        CHECK(reset_pc == 0x00123456u);
        CHECK(reset_ssp == 0x0010F300u);
        CHECK(ng_neogeo_watchdog_resets() == 1u);
        CHECK(!ng_neogeo_watchdog_reset_pending());
        CHECK(ng_neogeo_watchdog_last_kick_poll() == ng_neogeo_interrupt_polls());
        CHECK(ng_neogeo_watchdog_max_gap_polls() == 3u);
        CHECK(ng_neogeo_bios_vectors_enabled() == 0u);
        CHECK(ng_neogeo_backup_ram_unlocked() == 0u);
    }
    ...
    CHECK(!ng_m68k_take_interrupt(7, &level, &vector));
    ng68k_write8(NG_NEO_REG_DIPSW, 0x00u);
    CHECK(ng_neogeo_watchdog_max_gap_polls() == 2u);
    CHECK(!ng_m68k_take_interrupt(7, &level, &vector));
    CHECK(!ng_m68k_take_interrupt(7, &level, &vector));
    CHECK(!ng_neogeo_watchdog_reset_pending());
    ng_neogeo_reset_runtime();
    ...
    CHECK(ng68k_read8(NG_NEO_REG_POUTPUT) == 0xFFu);
    ...
    CHECK(ng68k_read8(0x00390021u) == 0xFFu);
    ng_neogeo_reset_runtime();
    CHECK(ng_neogeo_port_output() == 0x00u);
    ...
    CHECK(ng_neogeo_palette_bank() == 0u);
    ...
    CHECK(ng_neogeo_palette_bank() == 1u);
    ...
    CHECK(ng_neogeo_backup_ram_last_addr() == 0u);
    CHECK(ng_neogeo_backup_ram_last_value() == 0u);
    CHECK(ng_neogeo_backup_ram_nonzero_bytes() == 0u);
    CHECK(ng_neogeo_backup_ram_checksum() == 0u);

    ng68k_write16(0x00D00000u, 0x1111u);
    CHECK(ng68k_read16(0x00D00000u) == 0x0000u);
    ng68k_write8(0x003B001Du, 0x00u);
    CHECK(ng_neogeo_backup_ram_unlocked() == 1u);
    ...
    CHECK(ng_neogeo_backup_ram_last_addr() == 0x00D00001u);
    CHECK(ng_neogeo_backup_ram_last_value() == 0xFEu);
    CHECK(ng_neogeo_backup_ram_nonzero_bytes() == 2u);
    CHECK(ng_neogeo_backup_ram_checksum() == 0x01C8u);
    ng68k_write8(0x00D00003u, 0x5Au);
    CHECK(ng68k_read8(0x00D00003u) == 0x5Au);
    CHECK(ng_neogeo_backup_ram_write_count() == 3u);
    CHECK(ng_neogeo_backup_ram_last_addr() == 0x00D00003u);
    CHECK(ng_neogeo_backup_ram_last_value() == 0x5Au);
    CHECK(ng_neogeo_backup_ram_nonzero_bytes() == 3u);
    CHECK(ng_neogeo_backup_ram_checksum() == 0x0222u);
