...
`MOVE.W #imm,abs`, `MOVE.B #imm,abs`, `MOVE.B Dn,abs`, `CLR.B/W/L abs`,
`CLR.B Dn`, `TST.B abs`, `LEA`, and `MOVE.L An,abs` through the same generated
dispatch shape used by real ROMs. It also covers both not-taken and taken
`BNE`/`BEQ` paths, comparing generated C behavior against the tiny interpreter
oracle.
