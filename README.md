...
clean with `function candidates: 52253` and
`sites=8863 missing_direct=0 computed=0 runtime_computed=60`. The full test
...
object-state initializer misses at `$03FC38`/`$04D70C`/`$04F46A`, and the
tiny SFX/spawn helper misses at `$04FA40`/`$04FA58`, and the sprite-attribute
...
sound-latch diagnostics, a temporary fixed 50us post-command Z80 service slice
