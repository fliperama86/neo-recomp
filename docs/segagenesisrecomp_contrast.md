...
| Interrupts/STOP | Basic runtime-approved interrupt wake from `STOP` is covered; general instruction-boundary interrupts and NeoGeo IRQ sources remain open. | Runtime has cooperative VBlank and STOP yield hooks, but many semantics are tuned to Genesis/Sonic. | Use it as a design reference for runtime scheduling, not as a 68000 spec oracle. |
...
1. Add **general interrupt recognition** beyond the basic `STOP` wake path:
   instruction-boundary polling, level-7/IPL details, and NeoGeo IRQ sources.
