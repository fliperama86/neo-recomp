...
| Condition codes | Partial | `N`/`Z` and selected `C`/`X`/`V` paths are implemented where tests cover them, including byte `ADD` overflow/carry, byte `SUB` borrow, byte `CMP` overflow with `X` preserved, register-count-zero `LSL`, `ROL`, and `ROXL` flag behavior, plus BCD sticky-`Z`/`X`/`C` behavior for register and predecrement-memory cases. | Oracle/fuzz coverage for exact `X/N/Z/V/C` behavior across remaining arithmetic sizes/EAs, logical, shifts/rotates, remaining BCD/NBCD cases, compare, extend, and divide edge cases. |
| Arithmetic/divide semantics | Partial | `MULS/MULU`, `DIVS/DIVU`, `ADD/SUB/CMP`, extend arithmetic, BCD, and unary RMW paths exist. `DIVS/DIVU` divide-by-zero now enters the shared exception-frame path and honors trace-after-instruction-trap priority. A signed `DIVS` quotient-overflow fixture covers the unaffected-destination and `V`/`C`/`X` behavior for `$80000000 / -1`; generated-exec now covers selected byte `ADD`/`SUB`/`CMP` flag edge cases; BCD fixtures cover register and predecrement-memory `ABCD`/`SBCD` decimal carry/borrow behavior. | Exact overflow, quotient/remainder, undefined/unaffected flags, remaining `NBCD`/BCD edge cases, and broader arithmetic behavior against a trusted 68000 oracle. |
