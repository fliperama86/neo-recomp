...
| Basic trace exception entry | Done | `tests/test_c_emitter.c`, `tests/test_generated_exec.c`; generated `ng_service_trace()` helper. | Generated code snapshots the SR T bit before executing each instruction. Linear fall-through instructions, taken `BRA`/`Bcc`, taken `DBcc`, `JSR`/`BSR`/`JMP`, `RTS`, `STOP`, `RTE`/`RTR`, `TRAP`, and taken `TRAPV` paths now vector through trace vector 9 after execution when trace was enabled at instruction start. Trace stacks the architectural next/target PC, or for traced instruction exceptions the exception-handler PC after that exception frame is built, plus current saved SR on `SSP`, and clears the live T bit during exception entry. |
...
| Trace mode | Partial | Generated code uses the T bit from instruction start and services trace exceptions after linear fall-through, taken branches/`DBcc`, subroutine calls, jumps, `RTS`, `STOP`, `RTE`/`RTR`, `TRAP`, and taken `TRAPV`, before the next instruction-boundary interrupt poll. Exception entry clears the live T bit while preserving saved SR. | Trace priority around remaining instruction-generated exceptions (`CHK`, illegal/privilege) and pending exception interactions. |
...
| Full trace priority coverage | Missing | Complete trace behavior around remaining instruction-generated exceptions (`CHK`, illegal/privilege) and exception-priority interactions. |
...
2. **Full trace priority coverage**: add tests for remaining instruction-generated
   exceptions (`CHK`, illegal/privilege) and pending-interrupt priority.
