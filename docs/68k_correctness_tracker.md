...
| Runtime P-ROM and work-RAM bus slices | Done | `tests/test_runtime_interrupts.c`; `ng_neogeo_set_program_rom()`, `ng68k_read*()`, and `ng68k_write*()` in `runtime/src/neogeo_runtime.c`. | The default runtime can back fixed P-ROM reads, the repo's banked P-ROM window, 64KiB work RAM reads/writes, big-endian word/long helpers, and 24-bit address wrapping for those regions. P-ROM writes are ignored/read-only. Broader NeoGeo device regions remain partial/missing. |
...
| Runtime bus boundary | Partial | Generated code uses `ng68k_read*`/`ng68k_write*` and runtime hooks. The default runtime now covers fixed/banked P-ROM reads, 64KiB work RAM, selected LSPC/IRQACK registers, and 24-bit wrapping for covered regions. | Correct remaining NeoGeo memory map/device behavior: palette RAM, backup/system RAM, watchdog, inputs, sound latch/Z80 communication, BIOS/system registers, VRAM-facing ports, protection/bank-switch variants, and unmapped/bus-error policy. |
...
| NeoGeo hardware integration | Missing | Implement/test palette RAM, backup/system RAM, watchdog, inputs, sound latch/Z80 communication, BIOS/system registers, VRAM-facing ports, protection/bank-switch variants, VBlank/level integration with host pacing, and remaining unmapped/bus-error behavior. |
