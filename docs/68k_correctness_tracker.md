...
| Game TOML discovery seed parsing | Done | `tests/test_game_config.c`, `tests/test_function_discovery.c`; `recompiler/src/game_config.c`, `recompiler/src/function_discovery.c`, and `recompiler/src/main.c`. | `--game` now parses `[functions].entry`, `[functions].extra`, and `[game].discovery_files`; discovery-file paths are resolved relative to their parent config and merged into the same seed set with the cartridge entry while unmapped seeds are ignored. |
...
5. **Dispatch and metadata audit enforcement**: expand `games/*.toml` beyond
   function/discovery-file seeds, broaden dispatch-audit target patterns, and
   fail smoke runs on unresolved/missing control-flow gaps.
