...
| Condition codes | Partial | `N`/`Z` and selected `C`/`X`/`V` paths are implemented where tests cover them, including byte `ADD` overflow/carry, byte `SUB` borrow, byte `CMP` overflow with `X` preserved, address-register `ADDQ`/`SUBQ` and `ADDA`/`SUBA` CCR preservation, `CMPA.W` preserving `X` while updating `N/Z/V/C`, register-count-zero `LSL`, `ROL`, and `ROXL` flag behavior, `ABCD`/`SBCD` sticky-`Z`/`X`/`C` behavior for register and predecrement-memory cases, and `NBCD` data-register sticky-`Z`/borrow behavior. | Oracle/fuzz coverage for exact `X/N/Z/V/C` behavior across remaining arithmetic sizes/EAs, logical, shifts/rotates, remaining BCD/NBCD EA cases, compare, extend, and divide edge cases. |
| Arithmetic/divide semantics | Partial | `MULS/MULU`, `DIVS/DIVU`, `ADD/SUB/CMP`, extend arithmetic, BCD, and unary RMW paths exist. `DIVS/DIVU` divide-by-zero now enters the shared exception-frame path and honors trace-after-instruction-trap priority. A signed `DIVS` quotient-overflow fixture covers the unaffected-destination and `V`/`C`/`X` behavior for `$80000000 / -1`; generated-exec now covers selected byte `ADD`/`SUB`/`CMP` flag edge cases, address-register `ADDQ`/`SUBQ` full-32-bit no-CCR behavior, and `ADDA.W`/`SUBA.W`/`CMPA.W` word-source sign-extension with the required CCR effects; BCD fixtures cover register and predecrement-memory `ABCD`/`SBCD` decimal carry/borrow behavior plus `NBCD` data-register sticky-zero/borrow behavior. | Exact overflow, quotient/remainder, undefined/unaffected flags, remaining `NBCD`/BCD EA cases, and broader arithmetic behavior against a trusted 68000 oracle. |
