...
| Runtime P-ROM, work-RAM, palette-RAM, and backup-RAM bus slices | Done | `tests/test_runtime_interrupts.c`; `ng_neogeo_set_program_rom()`, `ng68k_read*()`, and `ng68k_write*()` in `runtime/src/neogeo_runtime.c`. | The default runtime can back fixed P-ROM reads, the repo's banked P-ROM window, 64KiB work RAM reads/writes, banked/mirrored palette RAM at `$400000-$7FFFFF`, palette bank selects at `REG_PALBANK1/0`, palette byte-write duplication into both bytes of a word, MVS backup RAM at `$D00000-$DFFFFF`, `REG_SRAMLOCK/UNLOCK` write gating, big-endian word/long helpers, and 24-bit address wrapping for covered regions. P-ROM writes are ignored/read-only. Broader NeoGeo device regions remain partial/missing. |
...
| Runtime bus boundary | Partial | Generated code uses `ng68k_read*`/`ng68k_write*` and runtime hooks. The default runtime now covers fixed/banked P-ROM reads, 64KiB work RAM, banked/mirrored palette RAM, MVS backup RAM with lock/unlock writes, selected system/LSPC/IRQACK registers, and 24-bit wrapping for covered regions. | Correct remaining NeoGeo memory map/device behavior: watchdog, inputs, sound latch/Z80 communication, BIOS/system registers/ROM, VRAM-facing ports, protection/bank-switch variants, persistent save backing, and unmapped/bus-error policy. |
...
| NeoGeo hardware integration | Missing | Implement/test watchdog, inputs, sound latch/Z80 communication, BIOS/system registers/ROM, VRAM-facing ports, protection/bank-switch variants, persistent save backing, VBlank/level integration with host pacing, and remaining unmapped/bus-error behavior. |
...
7. **NeoGeo bus slices**: continue implementing one hardware-visible region
   at a time with a failing runtime/generated-exec test first; palette RAM and
   basic MVS backup RAM are now covered, next candidates are BIOS/system
   registers/ROM, inputs, sound latch/Z80 communication, watchdog, and
   VRAM-facing ports.
