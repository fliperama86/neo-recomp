...
| Condition codes | Partial | `N`/`Z` and selected `C`/`X`/`V` paths are implemented where tests cover them, including byte `ADD` overflow/carry, byte `SUB` borrow, byte `CMP` overflow with `X` preserved, `CMPM.B` memory-compare `N/V/C` with `X` preserved, address-register `ADDQ`/`SUBQ` and `ADDA`/`SUBA` CCR preservation, `CMPA.W` preserving `X` while updating `N/Z/V/C`, register-count-zero `LSL`, `ROL`, and `ROXL` flag behavior, `ABCD`/`SBCD` sticky-`Z`/`X`/`C` behavior for register and predecrement-memory cases, and `NBCD` data-register plus predecrement-`A7` sticky-`Z`/borrow behavior. | Oracle/fuzz coverage for exact `X/N/Z/V/C` behavior across remaining arithmetic sizes/EAs, logical, shifts/rotates, remaining BCD/NBCD EA cases, compare, extend, and divide edge cases. |
