...
| Basic trace exception entry | Done | `tests/test_c_emitter.c`, `tests/test_generated_exec.c`; generated `ng_service_trace()` helper. | Linear fall-through instructions, taken `BRA`/`Bcc` paths, and taken `DBcc` paths with the SR T bit set now vector through trace vector 9 after execution, stack the next instruction PC or taken target PC plus saved SR on `SSP`, and clear the live T bit during exception entry. |
...
| Trace mode | Partial | Linear fall-through generated code, taken `BRA`/`Bcc` paths, and taken `DBcc` paths service trace exceptions after executed instructions and before the next instruction-boundary interrupt poll. Exception entry clears the live T bit while preserving saved SR. | Trace priority around instruction-generated exceptions (`TRAP`, `TRAPV`, `CHK`, illegal/privilege), `STOP`, and subroutine/return control-flow paths. |
...
| Full trace priority coverage | Missing | Complete trace behavior around `STOP`, subroutine/return control flow, instruction-generated exceptions, and exception-priority interactions. |
...
2. **Full trace priority coverage**: add tests for `STOP`, subroutine/return
   control flow, instruction-generated exceptions, and pending-interrupt
   priority.
3. **Trusted oracle/fuzz harness**: compare generated C against an external or
   embedded 68000 reference for per-instruction semantic edge cases.
4. **Real-ROM smoke refresh**: regenerate Metal Slug C, capture the current first
   frontier, add a regression, and update this tracker.
5. **Broaden post-decode legality validator**: make invalid source/destination
   effective-address combinations and CPU-family scope explicit for every
   decoded instruction family.
6. **NeoGeo bus slices**: implement one hardware-visible region at a time with a
