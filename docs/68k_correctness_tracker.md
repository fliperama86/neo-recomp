...
| Basic trace exception entry | Done | `tests/test_c_emitter.c`, `tests/test_generated_exec.c`; generated `ng_service_trace()` helper. | Generated code snapshots the SR T bit before executing each instruction. Linear fall-through instructions, taken `BRA`/`Bcc`, taken `DBcc`, `JSR`/`BSR`/`JMP`, `RTS`, `STOP`, and `RTE`/`RTR` paths now vector through trace vector 9 after execution when trace was enabled at instruction start, stack the architectural next/target PC plus current saved SR on `SSP`, and clear the live T bit during exception entry. |
...
| Trace mode | Partial | Generated code uses the T bit from instruction start and services trace exceptions after linear fall-through, taken branches/`DBcc`, subroutine calls, jumps, `RTS`, `STOP`, and `RTE`/`RTR`, before the next instruction-boundary interrupt poll. Exception entry clears the live T bit while preserving saved SR. | Trace priority around instruction-generated exceptions (`TRAP`, `TRAPV`, `CHK`, illegal/privilege) and pending exception interactions. |
...
| Full trace priority coverage | Missing | Complete trace behavior around instruction-generated exceptions and exception-priority interactions. |
...
2. **Full trace priority coverage**: add tests for instruction-generated
   exceptions and pending-interrupt priority.
