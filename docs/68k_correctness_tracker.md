...
| Effective-address legality | Partial | Generic EA helpers, many decode/emission paths, and a growing post-decode validator exist. Recent validator coverage includes immediate logical/arithmetic data-alterable destinations, register/memory source-destination shapes for `ADD`/`SUB`/`CMP`/`OR`/`AND`/`EOR`, and bit-op alterability/size checks. | Expand the validator to every remaining instruction family's legal source/destination EA combinations and verify all valid EA forms per family. |
...
1. **CPU-cycle-tied LSPC timer / VBlank integration**: connect the covered
...
7. **NeoGeo bus slices**: continue implementing one hardware-visible region
